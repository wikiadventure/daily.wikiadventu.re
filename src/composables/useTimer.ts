...
export function start(timeController:Ref<AbortController>, timeStamp:Ref<number>, time:Ref<number>, timeLeft?:number) {
  timeController.value.abort();
  timeController.value = new AbortController();
  timeStamp.value = document?.timeline.currentTime || performance.now();
  if (timeLeft) {
    time.value = formatTimer(timeLeft);
    animationInterval(100, timeStamp.value, timeController.value.signal, t => {
      const left = timeLeft - (t - timeStamp.value)
      time.value = formatTimer(left);
      if (left <= 0) {
        timeController.value.abort();
        timeController.value = new AbortController();
      }
...
export function useTimer() {
  const timeController = ref(new AbortController());
  ...
  return {
    ...
    timeStamp,
    time,
    startTimer: (timeLeft?:number) => start(timeController, timeStamp, time, timeLeft)
  }
